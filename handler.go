...
func MeetingShow(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	var meetingId int
	var err error
	if meetingId, err = strconv.Atoi(vars["meetingId"]); err != nil {
		panic(err)
	}
...
func MeetingCreate(w http.ResponseWriter, r *http.Request) {
	var meeting Meeting
	...
	t := Create_meeting(meeting)
	w.Header().Set("Content-Type", "application/json; charset=UTF-8")
	w.WriteHeader(http.StatusCreated)
	if err := json.NewEncoder(w).Encode(t); err != nil {
		panic(err)
	}
	fmt.Fprintln(w, "Meetings Scheduled!")
}
